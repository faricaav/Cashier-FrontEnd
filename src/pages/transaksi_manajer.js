...
export default function TransaksiManajer(){
    ...
    const getTransaksiByUser = (item) => {
        axios
        .get("http://localhost:8080/transaksi/byUser/"+item.id_user, headerConfig())
        .then((response) => {
          const dataRes = response.data
          console.log(dataRes)
          setTransaksi(dataRes)
    ...
    const filterDate = (e) => {
      ...
      let form = {
          tgl_transaksi_awal: tanggal_awal,
          tgl_transaksi_akhir: tanggal_akhir
      }
        ...
        <div className={`card mx-auto col-11 mt-5 mb-5 rounded-3`} class="no-printme">
          {/* begin::Header */}
            ...
            <div className='card-header border-0 pt-3 flex items-center mx-5'>
                <h3 className='card-title align-items-start col-8'>
                ...
                <div className='card-toolbar text-end'>
                    <Dropdown>
                        ...
                        <Dropdown.Menu>
                            <Dropdown.Item onClick={getTransaksi}>All</Dropdown.Item>
                            <Dropdown.Divider />
                            ...
                            {user.map((item, index) => {
                                return (
                                    <Dropdown.Item onClick={() => getTransaksiByUser(item)}>{item.nama_user}</Dropdown.Item>
                                )
                            })}
                        </Dropdown.Menu>
                    </Dropdown>
